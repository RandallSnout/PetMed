...
# ------------------------------------------------
	...
	def users_profile 
		...
		@a = Vet.joins(:users, :address).select("vets.id as VETS_ID", "vets.first_name as VETS_FN","vets.last_name as VETS_LN", "vets.email", "users.id as USER_ID", "users.first_name as USERS_FN", "users.last_name as USERS_LN", :street, :city, :state, :city).find_by("user.id == #{current_user.id}")
	end
